...
Open Growth 是一个面向社交媒体运营与增长流程的开源工作区，由 Workspace Project 管理、Content Repository、Publish、Tracking、Trends 五个工作流组成。
...
仓库遵循 ADR-0001 中记录的 monorepo 架构，并在该分层内实现完整产品能力：

- `apps/web`：React 19 + Vite 前端工作台，使用 React Router 管理 Repository、Publish、Tracking、Trends 页面。
- `apps/api`：Fastify 5 API 与静态资源服务，承载项目管理、素材管理、发布、追踪、趋势检索、认证鉴权和异步任务入口。
- `packages/shared`：前后端共享类型与领域对象。
- 工程化：TypeScript、Biome、Vitest。
- 存储：所有用户数据必须通过 Supabase Postgres、Supabase Auth 和 Supabase Storage 持久化，不能以 API 进程本地文件作为长期存储。

### 2.2 系统架构

系统架构保持 monorepo 分层，不引入新的前端框架或后端框架。
...
后端不能使用纯文件存储保存用户数据。系统直接使用 Supabase：Supabase Postgres 作为结构化用户数据的唯一事实来源，Supabase Auth 作为用户认证系统，Supabase Storage 作为多媒体文件存储。
...
3. Supabase Storage
   - 保存图片、视频、附件等多媒体二进制。
   - Storage object path 必须包含 `workspace_id/project_id/asset_id`，用于权限管理、清理和对象归属追踪。
...
项目规模不大，因此数据模型一次性定义为完整产品形态。设计原则是：表可以适度完整，但每张表必须对应独立业务事实，避免一对一无意义拆表。
...
- `workspace_members`
  ...
  - 每个 Workspace 必须至少包含一条 owner membership。
  - 字段包含 `workspace_id`、`user_id`、`role`、`created_at`。
  - RLS 统一基于 membership 判断访问权限。
...
系统使用 Supabase Auth 作为唯一用户认证系统。Open Growth 不自建密码系统，不在业务数据库中保存密码哈希。
...
3. Workspace 权限
   ...
   - 产品权限使用 owner role，RLS 和 schema 统一通过 membership 判断权限。
   - Project、Content、Publish、Tracking、Trends 的权限边界都必须继承 Workspace membership。
...
| 功能板块 | 核心价值 | 必须实现的能力 |
| --- | --- | --- |
| 内容管理 | 管理 Project 内的增长素材 | 素材上传、预览、删除、标签、搜索、筛选、草稿关联 |
| 内容发布 | 将素材和文案发布到目标平台 | 草稿、平台适配、立即发布、排期发布、失败重试、发布记录 |
| Engagement 追踪 | 追踪已发布内容的互动表现 | 指标拉取、快照存储、聚合展示、手动刷新、自动刷新 |
| Trending 功能 | 发现全网相关帖子和推广机会 | 关键词配置、平台检索、相关性筛选、状态管理、响应草稿创建 |
...
1. Project-scoped 素材库
   ...
   - 素材隔离必须通过数据库中的 `project_id` 和 Workspace membership 实现。
...
3. 素材预览
   - 图片显示缩略图。
   - 视频显示内嵌播放器、文件名和基础媒体信息。
   - 文本显示前若干行预览。

4. 素材元数据
   - 每个 Content asset 包含文件名、类型、大小、更新时间、预览信息。
   - 每个 Content asset 包含标题、描述、标签、来源、适用平台、关联 Project、关联 Published content、使用次数。
...
后端提供 `/api/content-assets` 作为 Content Repository API，由 `database-store` 保存 Supabase Postgres 元数据，由 `media-storage` 将多媒体写入 Supabase Storage：

- `GET /api/content-assets`：返回当前 Project 的素材列表。
...
`published-content-store` 模块按 Project 保存发布数据。领域模型：
...
`engagement-tracker` 模块依赖 `published-content-store` 和 `connector-service`。
...
2. 多平台检索
   ...
   - 平台不提供官方 API 时，可以使用手动导入、RSS、公开搜索或外部搜索 API adapter，但必须封装在 connector 内，并在 UI 中清楚标记数据来源和能力限制。
...
4. 作为增长运营者，我想给素材加标签和描述，这样可以快速检索和复用。
...
12. 核心模块具备 colocated Vitest unit test；integration test 放在各 package 根目录的 `integration/`；数据库相关测试放在 `packages/db` 内。
...
5. X、Reddit、Hacker News、小红书、微信都必须通过 `connector-service` 暴露统一接口；平台能力不足时，connector 必须用合规 adapter 明确表达支持范围、数据来源和失败状态。
6. 小红书与微信 connector 必须遵守官方 API、平台条款和合规边界；不使用明文凭据、前端凭据注入或绕过平台限制的抓取方式。
7. Supabase、媒体存储、outbox 和 connector 架构必须补充 ADR，记录边界、取舍和不可违反的约束。
8. Project-scoped data access context 是后端模块的稳定接口；业务模块不得自行拼接本地文件路径或绕过 `workspace_id/project_id` 权限边界。
