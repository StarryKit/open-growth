# ADR Index

Architecture decisions for this repo live here.

## Conventions

- Use numbered files like `0001-short-slug.md`.
- Start each ADR with status and date.
- Keep implementation plans explicit enough for an agent to follow.

## Index

- [ADR-0001: Record the current workspace architecture](./0001-current-workspace-architecture.md)
- [ADR-0002: Supabase domain storage, media, and outbox boundaries](./0002-supabase-domain-storage-and-outbox.md)
